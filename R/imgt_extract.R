...
imgt_extract <- function(.x, .start = 108, .end = 112, .rm_non_canonicals = FALSE, .aligned = FALSE) {
  ...
  #       })
  #
  #       junctions <- x[[which(junction_col)]]
  ...
  #       x <- imgt_extract_internal(x, start, end, rm_non_canonicals, aligned)
  #       return(x)
  #     }
  #
  ...
  if (inherits(x, "list")) {
    rlang::abort("Lists are currently not supported. See ?lapply() or ?purrr::map() to recurse on a list.")
  }
